...
[`../standards/README.md`](../standards/README.md) is the standards registry: every standard, code and union training programme this platform teaches against, written down once with its publishing body, its title, the catalog categories it governs and whether the citation form is verified or carried as a body and title only. It is the data (`tools/standards.json`) the content eval scores against and `tools/check_standards.mjs` gates on, and it lists what governs each training programme.
...
1. 29 automated checkers (`node tools/check_all.mjs`; the count is the `CHECKERS` list in that file and is printed on its last line): module parse and imports, every control reachable and crew figures clear of the work, mesh budget per headset frame, every interruption fires, times out, scores and visibly changes the scene, crew roles, incident replay, programme resolution, catalog freshness, accessibility, and every station citing a registered standard in scope for its category.
2. A headless-browser drive of every step with every interruption answered, and a spawn screenshot a person looks at.
3. The graded content review (`node tools/eval_content.mjs`): variety of interaction, decision density, explanation depth, grounding in named bodies, standards (the share of a station's cited authorities that resolve to a registry entry governing that station's category, less a penalty for citing out of scope), feedback coverage, scene, originality. The corpus mean and every station's row are kept in `tools/eval-content.json`.
...
OSHA 29 CFR 1910 (general industry) and 1926 (construction), 1915/1917/1918 (maritime), Cal/OSHA title 8, NFPA (70E, 70, 25, 96, 101, 1126, 1006), ANSI (Z359, A10, Z49.1, Z117.1), ASME B30, AWS D1, EPA and 40 CFR, 33 CFR and the IMO conventions for ballast and bunkering, the California Retail Food Code, the ABC Act and RBS programme, the Labor Code and Wage Orders for hospitality, CDC and ADA guidance for dental, 45 CFR 46 for biomonitoring, and the unions named on every station. Each of these lives as a registry entry with its scope; the full list is in [`../standards/README.md`](../standards/README.md).
