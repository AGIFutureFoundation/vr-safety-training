...
1. Twenty-five automated checkers (`node tools/check_all.mjs`): module parse and imports, every control reachable and crew figures clear of the work, mesh budget per headset frame, every interruption fires, times out, scores and visibly changes the scene, crew roles, incident replay, programme resolution, catalog freshness, accessibility, and every station citing a registered standard in scope for its category.
