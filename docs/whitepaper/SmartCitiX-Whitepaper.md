...
By the numbers, as the catalog records them on 2026-09-23: **235 stations** (226 SmartCiti.X stations, of which two are flat sourced briefings, plus the 9 rooms of the sibling Trade Skills Simulator) across **17 trade categories**, organised into **24 training programmes**, each naming a real union and the standard the block maps to. Together they hold **3,083 scored steps**, **957 seeded hazards** and **462 interruptions**. Every station cites at least one entry in a **standards registry of 285 entries across 75 publishing bodies**, of which 242 carry a citation form the authors are sure of and 43 are carried as body and title only, marked unverified. A **competency layer of 34 competencies** over 223 distinct stations and 69 standards sits above the game badges and applies one stated mastery rule. **30 headless checkers** gate every commit; the content evaluation ranks the corpus at a **mean of 95 out of 100** over all 235 procedures. **33 head-worn device profiles** in five classes decide how a station renders on hardware from a Quest to a hardhat monocular. Three input layers — keyboard, gamepad and voice — are checked so that nothing spoken can complete a step. A **flow layer** lets a host platform hand over an ordered graph of stations, gates and check-ins and hear back every transition; it is written as the SmartCiti.X side of a contract that Cognition.X's FlowHub, or any host, would implement.

What it proves today is narrower than what it contains, and the repository is careful to say so. Records are held in the learner's browser until exported as CSV, xAPI 1.0.3 or Open Badges 2.0; the badges are self-asserted until a hall hosts them. Identity is a launch context handed in by an LMS or a URL, not authentication; the one server in the repository, an LTI 1.3 relay, verifies a signed launch and has been proven against a stand-in platform, not registered with a real one. The practitioner review packet shows an automated preview pass on every section and a practitioner sign-off on none. No headset performance pass has been recorded, though the instrument to record one exists. The adaptive-content client (Orbis-Stable) and the Holodeck's prompt interpreter are seams with no live model behind them. The flow contract is specified from this side only: no Cognition.X end has been tested against it. A homepage with sign-in options verified on the host is in progress and not yet in the repository. The running app is published as a private page, shared on request.
...
|---|---|---|
| Stations in the shared catalog | 235 (226 SmartCiti.X + 9 Trade Skills rooms) | `catalog.json` |
...
| Scored steps across the roster | 3,083 (median 13 per station; range 10–16) | `catalog.json` |
| Seeded hazards | 957 | `catalog.json` |
| Interruptions | 462 across 231 procedures, every one visibly changing the scene | `check_interrupts.mjs` |
...
| Headless checkers | 30, all passing | `tools/check_all.mjs` |
| Content-eval corpus mean | 95 / 100 over 235 procedures | `tools/eval_content.mjs` |
| Standards registry | 285 entries, 75 bodies; 242 verified citation form, 43 unverified | `tools/standards.json` |
| Competencies | 34 (24 programme, 10 core) over 223 stations and 69 standards | `check_competency.mjs` |
| Flows | 3 example flows; platform protocol 2 (flow.load / start / resume / state / done / external); observer protocol 3 (`flow` command) | `WebXR/flows/`, `check_flowhub.mjs` |
...
Two rules are enforced by `tools/check_interrupts.mjs`: an interruption must visibly change the world (a fan stops and its lamp goes red; a lock is gone from the hasp) — "an alarm you can only read is a caption" — and its answer must not be the control the learner is already holding, because "that is not an interruption, it is a nudge." 462 interruptions across 231 procedures pass these rules today. The **Situational Awareness — Interruption Drill** programme collects 22 stations a learner may already know and reports an `attention` figure — interruptions caught against dropped — separately from the station count, because "passing the procedure and noticing the alarm are two different competencies."

### 3.4 Hazards

Every walkable station seeds four hazards on registered objects (the brief's requirement; the corpus holds 957). Touching one costs 50 points, is recorded as an unsafe action, and explains its consequence in text the accessibility checker requires to be a full sentence. In the two flat stations the hazards are unsafe conclusions rather than objects — "treating a parcel as open ground, swapping a clean sample, silencing a dust monitor, dismissing the community's monitors" — scored on the same engine. The instructor console can switch a session between **Assess** (an unsafe action counts) and **Coach** (called out once, taken back off the count); the choice is written onto the record as `hazardMode`.
...
**The platform channel** (`shared/platform.js`, now protocol 2) lets an LMS or portal drive an app (`open`, `hub`, `status`, `catalog`) and hear back (`ready`, `state`, `progress`, `record`, `credential`), only to the `learner_home` origin; protocol 2 adds the flow commands described in 3.18. The static catalog is the roster a platform can index "without loading the app at all."
...
- Every one of 226 SmartCiti.X simulators and 9 Trade Skills rooms parses, imports only what it declares, builds, and can be played to a perfect run on the real engine.
- Every one of 462 interruptions fires after its delay, times out if unanswered, counts the miss as an unsafe action, scores a correct answer, is disarmed when the learner leaves its step, and visibly changes the scene.
...
- **Identity and sign-in verification on the host.** The apps cannot verify who is in front of them. `identity.js`: "There is no server here, so nothing can be cryptographically verified: this is a *launch context*, not authentication." Verification is the host's job — an LMS that knows the learner, or the LTI relay verifying a signed launch. A homepage with sign-in options — Google and Microsoft through client ids, an email magic link or a device passkey, and Sign-In with Ethereum, each verified on the host and none inside the page — is being built now and is not yet in the repository; until it lands, a deployment provides identity from its own host side.
...
- **Extend the robot rule set across the whole dental block.** The Dental Careers programme is finished at sixteen stations; `tools/check_robot.mjs` still holds only the fifteen hygiene stations to the keep-out and `noRobot` rule set. Depends on: the dental rule set in `docs/robot-training.md`, already written, applied to the ten careers and five advanced stations and their ids added to the checker's roster.
- **The FlowHub contract with Cognition.X.** The SmartCiti.X side exists (section 3.18): the flow schema, the validator, the six message types on platform protocol 2, the Flows panel, console send and `check_flowhub.mjs`. It is written as the side of a contract that Cognition.X's FlowHub would implement or adapt to; no FlowHub specification is in the repository and no Cognition.X end has been tested against it. Depends on: a Cognition.X host that embeds the app, supplies identity from its own origin, posts `flow.load` and `flow.start`, and answers `flow.external` with `flow.resume` — then a test of a whole run and a refusal.
- **Homepage and sign-in — in progress.** A homepage with sign-in options is being built now: Google and Microsoft through client ids, an email magic link or a device passkey, and Sign-In with Ethereum, every one verified on the host and none inside the page, which keeps `identity.js`'s rule that the app itself never authenticates anyone. It is not yet in the repository; today the portal is a static map and identity is a launch context. Depends on: the host that verifies the sign-in handing the launch context to the apps, and, for an LMS, the LTI relay deployed behind TLS.
...
- **A verified standards library.** 43 registry entries are unverified, most of them union training-fund course names. Depends on: confirming citation forms with the bodies, and, separately, the practitioner review — the packet has 0 sign-offs, and a regenerated roster to cover all 235 stations.
...
**Can I embed it in our LMS, and can the host sequence stations?** Yes, through the platform channel: identity in, `progress`/`record`/`credential` events out, to your origin only, and — at protocol 2 — a flow in and every transition out. The static catalog lets you index every station without loading the app.

**What is the CI, and what about accessibility procurement?** `node tools/check_all.mjs` (30 checkers) plus a freshness check on generated files, on every push touching `WebXR/` or `tools/`. Accessibility: a self-assessed WCAG 2.1 AA / Section 508 statement for the flat mode, non-conforming headset modes listed, no third-party audit.

**Is there an account system or sign-in?** Not yet in the repository. A homepage with sign-in options (Google and Microsoft client ids, email magic link or device passkey, Sign-In with Ethereum) is being built; every option is verified on the host, none inside the page, which is the design.

### From an investor

**What is actually built?** 235 stations, 24 programmes, 34 competencies, a shared engine, records and credentials, 33 device profiles, a flow layer, an instructor console, a verifier, a robot embodiment layer and 30 checkers, in 226 commits over about nine weeks.

**What is not built?** The homepage and sign-in (in progress, host-verified), any hosted badge issuer, any registered LTI deployment, any practitioner-signed station, any headset performance data, any live model behind the Holodeck or Orbis-Stable seams, the Cognition.X end of the flow contract (only the SmartCiti.X side exists), any cohort store, any multi-user scene.

**Is there a host platform?** The repository names Cognition.X as the user's host platform and FlowHub as its orchestration component, and ships the SmartCiti.X side of a flow contract for it; it holds no FlowHub specification and records no test against a Cognition.X end.
...
**Do I need a headset?** No. The flat mode runs the same procedure, scores it the same way and writes the same record: a learner without a headset "is not on a lesser course."
...
**What does a competency mean for me?** That you ran named stations under the mastery rule and the transcript shows every run behind it. It evidences readiness for the standard named; it is not the certification itself.
...
| `WebXR/smartcity/` | SmartCiti.X: `index.html`, `js/app.js`, `js/sims/*.js` (226 stations), `js/curricula.js` (24 programmes), `js/citykit.js`, `js/districts.js`, `js/interiors.js`, `js/apron.js`, `js/ambient.js`, `js/gamify.js`, generated `js/sims-meta.js` and `catalog.json`, `REVIEW.md`, `models/guide-worker.glb`, `dist/` |
...
| `WebXR/shared/` | The engine (`game.js`) and the shared layers named through section 3: records, competency, identity, LRS, platform, flowhub, observer, devices, input, hands, a11y, voice, EI guide, weather, environment, kit, perf, robot, embodiment, lessons, incidents, variants, crew, Orbis-Stable |
...
| `tools/check_all.mjs`, `tools/check_*.mjs` | The 30 checkers |
...
3. `check_smartcity.mjs` — All 226 simulators pass.
...
16. `check_budget.mjs` — All 235 stations inside budget (320 meshes on the stage, 430 standalone). Fullest: welding 387/430, kitchen 385/430, salon 381/430.
17. `check_layout.mjs` — All 235 stations reachable.
18. `check_interrupts.mjs` — 462 interruptions across 231 procedures check out: the engine fires, times out and scores them, and all 462 visibly change the world.
...
28. `check_competency.mjs` — All competency checks pass. 34 competencies (24 programme, 10 core) · 223 distinct stations · 69 standards.
...
Final line: **All 30 checkers pass.**
