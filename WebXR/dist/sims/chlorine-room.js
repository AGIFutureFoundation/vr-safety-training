  ...
  steps: [
    {
      ...
      why: "The SOP names the setpoint the plant needs and the two-person rule for the room, and it exists because a chlorine cylinder change is not a job to improvise under time pressure — every line in it removes one way to end up standing in a room with a rising concentration and no plan already agreed to. The change is done the same way every time so the one time it leaks, the response is a habit already rehearsed, not a decision made from scratch with an alarm already sounding.",
    },
    {
      ...
      why: "The monitor is the only thing that smells chlorine before you do at a concentration that matters — the OSHA PEL sits well below the level a human nose reliably notices, so by the time you can smell it yourself the room may already be past the ceiling limit. It is read and bump-tested before the door opens every time, because a sensor that has drifted or died is worse than no sensor at all: it tells you the room is clear when it no longer has any way of knowing.",
    ...
    },
    {
      ...
      why: "Nobody works a chlorine cylinder alone in this room. The pack is staged at the door because the leak, if it comes, comes at the yoke where you are standing — Chlorine Institute Pamphlet 65's Emergency Kit A assumes a response starts within seconds of a release, not minutes, and an SCBA fetched from a locker down the hall after the alarm sounds is a rescue that has already lost the only window that mattered.",
    },
    {
      ...
      why: "The regulator comes off a closed valve, never an open one — cracking a fitting on a cylinder still under pressure sprays chlorine gas straight at whoever's hands are on the wrench. The cylinder reads 'empty' at roughly 20 lb residual, not zero, and that residual is still under enough vapour pressure to empty itself through any fitting left open, which is exactly why the valve is proven fully closed before anything downstream of it is touched.",
    ...
    },
    {
      ...
      why: "The cap seals the outlet and the hood protects the valve stem for the trip across the room and the ride back to the supplier — Chlorine Institute Pamphlet 1 treats an uncapped valve as the single failure that turns a dropped or rolled cylinder into a full release, because a snapped valve stem on a cylinder this size empties the entire remaining charge in well under a minute with nothing left to shut it off.",
    ...
    },
    {
      ...
      why: "The scale is how the plant knows what is actually left in service rather than trusting a gauge or a guess: feed is confirmed by weight loss over the shift, and a cylinder is called empty at roughly 20 lb residual rather than run to zero, because a cylinder emptied of usable chlorine still holds a vapour pressure that will happily push whatever residual remains straight out through any fitting left open.",
    ...
    },
    {
      ...
      why: "A chained cylinder cannot fall on its own valve, and a 150-lb cylinder falling on an unprotected valve stem is exactly the impact that snaps one off. A new lead gasket at every connection is the Chlorine Institute's own rule rather than a convenience, because a lead gasket seals by taking a single controlled crush — reusing one that has already taken that crush leaves a joint that looks tight and weeps chlorine the moment pressure comes up behind it.",
    },
    {
      ...
      why: "The lead gasket seals by taking a controlled crush, and a quarter turn past hand-tight is the amount of crush that seals the joint without going further: less than that and the gasket never fully seats, so it weeps chlorine at the yoke as soon as pressure comes on; more than that crushes the gasket into the valve threads and leaves a damaged sealing face that the next change inherits whether or not the gasket itself gets replaced.",
    ...
    },
    {
      ...
      why: "Ammonia vapour meets chlorine gas and makes a visible white ammonium chloride cloud at a concentration of only a few parts per million, well below what most people can smell and far below anything an eye could catch on a bare joint — which is why every yoke, every regulator fitting and every valve packing gets tested with the bottle before the cylinder goes into service, not just the ones that look suspect.",
    ...
    },
    {
      ...
      why: "One full turn of a chlorine cylinder valve gives full flow through the regulator; turning it further only adds turns that have to be found again by feel if the valve needs shutting fast in a leak. The wrench stays seated on the stem for exactly that reason — the first motion in an emergency has to be closing the valve, and a wrench that has to be located first is seconds a chlorine release in an enclosed room does not give back.",
      turn: { turns: 1, axis: "y", label: "CYLINDER VALVE" },
    },
    {
      ...
      why: "The setpoint is the chlorine residual the plant is required to hold at the customer's tap, not a number set to look busy on the rotameter: too little and pathogens the disinfection step exists to kill can survive the required contact time, too much and the plant fields taste-and-odour complaints while paying to feed chlorine that leaves through the vent as unused excess residual.",
    ...
    },
    {
      ...
      why: "The room is left the way the next leak — not this shift's leak, whichever one comes next — actually needs it: the wrench seated on the in-service valve stem where an emergency response starts, Kit A sealed and within reach rather than checked once a quarter, the door closed, and the monitor confirmed live rather than assumed live because it was live yesterday.",
    },
  ],
