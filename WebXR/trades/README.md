...
Five vocational training rooms in one WebXR demo. Each room is a working
...
- **Hazard objects carry consequences.** Every room seeds real traps: a metal
  ...
  cylinder. Selecting one costs double and explains what would have happened.
...
- **Timed steps must actually be held.** The 20-second handwash, the venipuncture
  site pressure and the post-weld fire watch all require a continuous hold;
  letting go early restarts them.
...
XP and stars persist per browser and show on the hub pillar and on each doorway.
...
- **Single file** (what you deploy): `dist/trade-skills-simulator.html`, generated
  by `python3 tools/bundle_trades.py` from the repository root. Copy it anywhere
  and serve it over HTTPS.
- **Deep link a single trade**: `?room=electrical` (or `salon`, `kitchen`,
  `phlebotomy`, `welding`) opens straight into that bay, so one room can be
...
cleanly. `tools/bundle_trades.py` additionally refuses to emit a bundle when two
...
js/hub.js           the atrium: five doorways, per-room record plaques
...
Content and engine behaviour are covered by `tools/check_trades.mjs` (all five
