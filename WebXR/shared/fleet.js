 ...
 * tailLights}, hood (tilts forward about X by userData.openAngle), fifthWheel, fifthWheelRelease, gladHandService, gladHandEmergency.
...
export function semiTractor(parent, x, y, z, opts = {}) {
  ...
  // Bumper, fenders, and the hood with its grille: a part hinged at the
  // bumper, so a station can tilt it forward for an engine-bay walk.
  box(S, 2.44, 0.36, 0.22, 0, 0.56, Z(0.11), ...FL.chrome);
  const hood = rig.part("hood", 0, 0.78, Z(0.16));
  flSide(hood, [[0.16, 0.78], [0.13, 1.5], [0.2, 1.76], [0.5, 1.9], [2.05, 2.08], [2.05, 1.0], [0.9, 1.0], [0.6, 0.78]].map(([b, y]) => [b - 0.16, y - 0.78]), 1.86, 0, 0, 0, ...P, { bevel: 0.06 });
  flPanel(hood, 1.2, 0.86, 0, 1.3 - 0.78, 0.06, flGrilleMat("truck"), "+z");
  hood.userData.openAngle = 1.35;               // about X: the top tips forward over the bumper
  for (const sx of [1, -1]) {
    const arc = [];
...
const FL_TRAILERS = {
  ...
  // A 28 ft pup: the short single-axle van LTL carriers pull as doubles, and
  // the length a station can park side-on inside the site apron's fence.
  pup: { L: 8.53, axles: [7.45], landing: 2.4 },
};
 ...
 * pockets and winches), "tanker" (42 ft DOT-406 style, elliptical barrel) or
 * "pup" (28 ft single-axle dry van).
...
export function trailer(parent, x, y, z, opts = {}) {
  ...
  const S = rig.shell;
  const W = 2.59;
  const van = kind === "dryVan" || kind === "reefer" || kind === "pup";
  const axMid = T.axles.reduce((a, b) => a + b, 0) / T.axles.length, axLast = T.axles[T.axles.length - 1];
  ...
  box(S, 1.0, 0.5, T.axles.length > 1 ? 2.6 : 1.4, 0, deckY - 0.45, Z(axMid), ...FL.frame);
  ...
  for (const sx of [1, -1]) box(S, 0.62, 0.6, 0.02, sx * 1.0, 0.5, Z(axLast + 0.7), ...FL.frame);
  ...
  if (van) {
    ...
    const side = flLiveryMat(lv, `van:${kind}`, { base: lv.colour, posts: Math.round(40 * L / 16.15), tape: true, pw: 2048, ph: 360, titleScale: 0.26, titleY: 0.36, stripeY: 0.6 });
...
export const FLEET_BUDGET = {
  semiTractor: { build: "semiTractor", meshes: 24, footprint: [3.02, 3.95, 6.86], parts: ["doorL", "doorR", "mirrorL", "mirrorR", "hood", "wheels", "lights", "headlights", "markerLights", "tailLights", "fifthWheel", "fifthWheelRelease", "gladHandService", "gladHandEmergency"], note: "Class 8 day cab, tandem drive, fifth wheel, air lines and glad hands" },
  "semiTractor:sleeper": { build: "semiTractor", opts: { cab: "sleeper" }, meshes: 24, footprint: [3.02, 4, 8.66], parts: ["doorL", "doorR", "mirrorL", "mirrorR", "wheels", "lights", "fifthWheel", "gladHandService", "gladHandEmergency"], note: "Class 8 raised-roof sleeper" },
  ...
  "trailer:pup": { build: "trailer", opts: { kind: "pup" }, meshes: 13, footprint: [2.63, 4.11, 8.65], parts: ["doorL", "doorR", "wheels", "landingGear", "gladHands", "lights"], note: "28 ft single-axle pup van" },
  tractorTrailer: { build: "tractorTrailer", meshes: 38, footprint: [3.02, 4.11, 20.71], parts: ["tractor", "trailer"], note: "day cab coupled to a 53 ft dry van" },
