...
export function bestRadioScore(storage) {
  try {
    const raw = JSON.parse(quizStorage(storage)?.getItem(SCORE_KEY) || "null");
...
export function recordRadioScore(score, of, storage) {
  ...
  try { quizStorage(storage)?.setItem(SCORE_KEY, JSON.stringify({ best, of, runs: (prev?.runs ?? 0) + 1 })); } catch (_) { /* private mode */ }
