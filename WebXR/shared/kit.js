...
export function floorPaint(parent, w, d, o = {}) {
  ...
  const m = decal(parent, w, d, 0, 0.012, 0, (g, cw, ch) => {
    ...
    else { g.fillStyle = "rgba(0,0,0,0)"; g.fillRect(0, 0, cw, ch); }
    // Walkway: two solid lines, symmetric so the decal reads the same either
    // way up (a rotated plane can present its canvas mirrored).
    const laneW = cw * (o.laneFrac ?? 0.34), lx = (cw - laneW) / 2;
    g.strokeStyle = hex(lane); g.lineWidth = Math.max(3, cw * 0.008);
    g.beginPath(); g.moveTo(lx, 0); g.lineTo(lx, ch); g.moveTo(lx + laneW, 0); g.lineTo(lx + laneW, ch); g.stroke();
    // Hazard hatch on both outer margins.
    const marg = cw * 0.075;
    g.strokeStyle = hex(hatch); g.lineWidth = Math.max(2, cw * 0.006);
    for (let i = -ch; i < ch * 2; i += cw * 0.055) {
      g.beginPath(); g.moveTo(0, i); g.lineTo(marg, i + marg); g.stroke();
      g.beginPath(); g.moveTo(cw, i); g.lineTo(cw - marg, i + marg); g.stroke();
    }
  }, { px: o.px ?? 1024, transparent: !base, rough: 0.9 });
