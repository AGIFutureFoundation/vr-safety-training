...
stand up. `WebXR/shared/platform.js` defines protocol 2 (a protocol-1 host is unaffected):
...
| host → app | `smartcitix:flow.load { flow }` | hand over a **flow**: an ordered graph of nodes with a condition on every edge (`shared/flowhub.js`). Validated against the catalog before it is stored, refused whole with the reasons if it does not check out |
| host → app | `smartcitix:flow.start { flowId?, restart? }` | begin a loaded flow and stand on its first node |
| host → app | `smartcitix:flow.resume { nodeId, outcome }` | the host finished the `external` node it was handed; accepted only for the node the flow is parked on |
| app → host | `smartcitix:flow.state { flow, transition }` | every transition, with the branch taken and the reason it was taken |
| app → host | `smartcitix:flow.external { flowId, nodeId, node }` | a node whose app is `host` — nothing moves here until a resume arrives |
| app → host | `smartcitix:flow.done { flow }` | the run reached a terminal node |
...
same channel is wired in Trade Skills Simulator (`room` instead of `sim`) and in Holodeck.

A flow chooses the order; it never decides a verdict. Every node's outcome is the attempt
record this engine already writes, and a competency gate is the proof brief's mastery rule
over those records. Three example flows ship in `WebXR/flows/`, the learner's own **Flows**
panel shows the path taken and why, and the instructor console can send one (`docs/flowhub.md`,
which states that it is the SmartCiti.X side of a host contract and claims nothing about any
host's internals).
