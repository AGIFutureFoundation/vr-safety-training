...
export const CURRICULA = [
  ...
  },
  {
    ...
    stations: [
      ...
      { app: "smartcity", id: "le-crowd-barricade-and-show-stop-call", why: "The front-of-house rail before doors: a bent pin caught, the line built and gauged to the plan, and the show-stop chain briefed and used when the load gauge spikes." },
    ],
  },
  {
  ...
  },
  {
    ...
    stations: [
      ...
      { app: "smartcity", id: "drive-city-route-and-turns", why: "The first time the learner drives the rig themselves: a block of right turns with the trailer behind, signalled early, swung late, kept tight to the curb and watched in the right mirror where the tandem cuts in." },
      { app: "smartcity", id: "drive-freeway-merge-and-following-distance", why: "Up the ramp and into traffic at the wheel, with the handbook's seconds-per-ten-feet space cushion counted, a lane change for the next merge, a car cutting in and an exit queue warned with the flashers." },
      { app: "smartcity", id: "drive-mountain-grade-and-engine-brake", why: "A long downgrade driven on the engine: brakes checked at the pull-out, the low gear chosen before the crest, snub braking instead of riding the pedal, and a brake-fade warning answered with the engine brake." },
      { app: "smartcity", id: "drive-night-fog-and-rail-crossing", why: "A placarded tank at night in fog: low beams, a speed inside the sight distance, the window down, the crossing stop 49 CFR 392 requires, and across both tracks in one gear without shifting." },
      { app: "smartcity", id: "drive-backing-serpentine-and-alley-dock", why: "The backing the yard station reads, driven: signals agreed with the spotter, a serpentine through the cones, a sight-side alley dock, and the truck stopped the moment the spotter leaves the mirror." },
      { app: "smartcity", id: "drive-light-vehicle-fleet-and-forklift-course", why: "The two check-rides a terminal gives before the keys: a pool car walked round, stopped fully and backed into its space, and a forklift driven load-low with the horn at every aisle end and in reverse behind a tall load." },
  ...
  },
  {
    ...
    certification: "USA Basketball youth development guidelines for age-appropriate play, practice and rest; NFHS basketball rules and sports medicine guidance; CDC Heads Up concussion training; U.S. Center for SafeSport abuse-prevention policies; the American Red Cross first aid, CPR and AED course",
    guides: ["afscme-training", "seiu-training", "usa-basketball-youth-guidelines", "nfhs-basketball-rules", "cdc-heads-up", "safesport-code", "red-cross-first-aid-course"],
    summary: "Ten stations on an indoor court that teach players the fundamentals and teach coaches to run a session nobody gets hurt in: the floor walked before anyone runs on it, contact introduced by size and stage, every knock to the head taken out of play, water and rest on the clock, and every heated moment, alarm and parent at the door handled calmly and in view.",
    accent: "#ff7a3b",
    stations: [
      { app: "smartcity", id: "bb-warmup-injury-prevention-and-hydration", why: "The first fifteen minutes that decide the rest: the floor walked dry, the emergency plan read, a dynamic warm-up in order and water called by the clock." },
      { app: "smartcity", id: "bb-stance-and-ball-handling", why: "The ball in the hands for the first time: balls checked, lanes spaced wide, the triple-threat stance built from the feet up and the eyes kept off the ball." },
      { app: "smartcity", id: "bb-footwork-pivots-and-jump-stops", why: "The lower body every later skill stands on: soft, balanced landings with knees over toes, a legal pivot on the ball of the foot and a jump count that is kept." },
      { app: "smartcity", id: "bb-passing-and-catching", why: "Two players and one ball in the air: a name before every pass, fingers up on every catch, speed matched to the receiver and any ball to the head taken seriously." },
      { app: "smartcity", id: "bb-shooting-form-and-arc", why: "The shot built from balance up on a hoop set to the player's age, with the arc and the release held in band and the lane under the rim kept clear." },
      { app: "smartcity", id: "bb-free-throw-routine", why: "One routine held the same way through fatigue and pressure, with a dry line, a lane that waits for the release, water between rounds and shooters rotated to rest." },
      { app: "smartcity", id: "bb-defensive-stance-and-closeouts", why: "The hardest work in practice done safely: room to slide, active hands instead of reaching, and closeouts chopped under control and never into a shooter's landing." },
  ...
  },
  {
    ...
    union: "ILWU and the Inlandboatmen's Union, MEBA and SIU marine crews, Pile Drivers Local 34 divers, IUOE Local 3 dredge and excavator operators, LIUNA Local 261 hazmat and shoreline crews, AFSCME and SEIU agency scientists",
    certification: "Programme completion record; the dive, vessel and hazmat credentials it rehearses are issued only by their own bodies — OSHA 29 CFR 1910 Subpart T and the ADCI consensus standard for the dive crew, USCG 33 and 46 CFR for the vessels, OSHA 1910.120 HAZWOPER inside any sediment exclusion zone, and the Section 404/401 permit conditions for every hour of in-water work",
    guides: ["liuna-training-fund", "iuoe-training", "carpenters-ictf", "ilwu-pma-training", "osha-1910-120", "usace-section-404", "bcdc-bay-plan", "rwqcb-401-certification"],
    ...
    accent: "#4fb3c8",
    stations: [
      { app: "smartcity", id: "br-dive-site-hazard-assessment-and-jsa", why: "Every restoration dive starts on deck with the supervisor's job safety analysis, where the intake, the neighbouring pile hammer, the current and the contaminated water are found and controlled before anyone dresses." },
      { app: "smartcity", id: "br-surface-supplied-dive-station-setup", why: "The station the diver breathes from is built by the dive team itself, so the compressor intake, the condensate, both supplies, the bailout and the helmet's non-return valve are proven before the first dive of the day." },
      { app: "smartcity", id: "br-underwater-debris-survey-and-mapping", why: "No debris comes off the Bay floor until a diver has mapped it on a baseline, found the drums, batteries, rebar and hulls, and left the hazardous items for the work plan." },
      { app: "smartcity", id: "br-underwater-sediment-core-sampling", why: "Dredging and capping decisions rest on sediment cores a diver takes by hand, so the cable crossing, a steady push, a capped core and the chain of custody are rehearsed with a bailout and a recall along the way." },
      { app: "smartcity", id: "br-derelict-gear-recovery-dive", why: "Ghost nets and lost pots keep killing what the restoration is bringing back, and recovering them means working from up-current, cutting away from the umbilical and keeping a lift bag neutral and never on the diver." },
      { app: "smartcity", id: "br-hyperbaric-chamber-standby", why: "A decompression dive is only as safe as the chamber standing by on deck, so the chamber operator rehearses the leak test, the fire load, venting before the door and blowing a symptomatic diver down while the physician is called." },
      { app: "smartcity", id: "br-dive-tender-and-umbilical-management", why: "The tender keeps the diver's gas, voice and lifting line in hand for the whole dive, from the harness check and a steady pay-out to a bight near a propeller, dressing the standby and flaking the umbilical afterwards." },
      { app: "smartcity", id: "br-derelict-vessel-salvage-rigging", why: "A crane barge alongside a wreck is work over water on a deck with no bulwark, and a person who goes over the edge in Bay water loses the use of their hands to the cold long before they tire \u2014 the work vest is what keeps their face out of the water until the skiff reaches them." },
      { app: "smartcity", id: "br-workboat-crane-lift-from-water", why: "The afterdeck of a small workboat is low to the water and wet, and a lift over the side means an open gate in the rail for most of the job." },
      { app: "smartcity", id: "br-debris-skimmer-vessel-operations", why: "The skimmer's working deck is forward of the wheelhouse, low, wet and open to the water between the hulls, and the person working it spends the job looking down at a belt and a basket rather than at the edge." },
      { app: "smartcity", id: "br-boom-towing-between-two-vessels", why: "The afterdeck of a boom-towing boat is low, wet and slick with product by the end of the job, and the tow line on it can move fast." },
      { app: "smartcity", id: "br-barge-loading-of-contaminated-sediment", why: "The barge deck is work over water and it is also inside the site's contamination zone, so both kinds of protection go on before the gangway." },
      { app: "smartcity", id: "br-cold-water-immersion-and-mob-recovery", why: "The first moments in cold water bring a gasp and a loss of breath control that a person cannot choose to stop, and a PFD is what keeps the mouth clear of the water through it." },
      { app: "smartcity", id: "br-vhf-and-navigation-in-a-work-zone", why: "The watch that keeps the whole zone safe on the water: the radio checked and the watch set on 16, the sécurité call that tells every bridge what is out here, the marker set where the chart puts it, a ferry crossing handled by the rules, and fog signals sounded as the bank comes over." },
      { app: "smartcity", id: "br-dredge-spoils-dewatering-pad", why: "Wet dredged sediment comes ashore into a lined pad, and the decant that drains from it goes back to the Bay only through a filter train under the pad's discharge conditions, sampled under custody and diverted the moment the effluent meter trips." },
      { app: "smartcity", id: "br-turbidity-curtain-deployment", why: "The curtain closes the water round a dredge before it digs, laid from upcurrent with its skirt set against the tide table and the shore end sealed, so the certification's compliance point has a boundary to be judged against." },
      { app: "smartcity", id: "br-sediment-chain-of-custody-and-lab-prep", why: "Cores become laboratory samples at a dockside bench with custody signed at every hand-off, the volatiles taken before anything is mixed and anything unlabelled kept out of the batch, because a sediment result is only as defensible as its record." },
      { app: "smartcity", id: "br-water-quality-sonde-calibration-and-deploy", why: "The compliance sonde is calibrated against in-date standards, locked at the plan's depth and checked side by side before its readings count, and when it trips the dredge is stopped on its say-so." },
      ...
      { app: "smartcity", id: "br-marine-mammal-observer-during-pile-driving", why: "A pile-driving crew can only work under NOAA Fisheries' own authorization when its observer scans the shutdown zone clean before every ramp-up and calls an immediate stop the moment a harbor seal or harbor porpoise is inside it." },
      { app: "smartcity", id: "br-bird-nesting-buffer-and-work-window", why: "An amphibious excavator staged on a restoration bench does not move until the compliance biologist has read the work-window calendar, confirmed the buffer by rangefinder and heard the operator's own acknowledgement of the flagged line." },
      ...
      { app: "smartcity", id: "br-native-planting-and-erosion-mats", why: "The graded bank above the marsh is only held until roots can hold it themselves, so the coir mat goes down pinned top to toe before a single native plug is bored, tamped and watered in." },
      { app: "smartcity", id: "br-culvert-retrofit-for-fish-passage", why: "A perched culvert that a fish cannot swim through only becomes passable on a dry, fish-excluded channel, dug and backfilled to a laser-checked invert instead of a grade that just looks about right." },
      { app: "smartcity", id: "br-shoreline-cleanup-sharps-and-hazardous-debris", why: "A storm-tide beach cleanup is ordinary trash until the moment it isn't, so sharps get tongs and a staged container, a suspect drum gets flagged and called in, and nothing this crew is not trained to handle ever gets opened by hand." },
      { app: "smartcity", id: "br-intertidal-invasive-removal-by-hand-crew", why: "With no sprayer on this crew at all, every plant comes out whole on a wrench, every root fragment gets hunted out of the disturbed soil, and boots get brushed clean at the buffer so nothing this crew pulls ever gets a second chance to spread." },
      { app: "smartcity", id: "br-levee-inspection-and-seepage", why: "A sand boil gets ringed with sandbags and never blocked, and a burrow gets flagged and reported and never dug at, because on a levee patrol the counterintuitive answer is usually the one that keeps a small seepage path from becoming a bigger one." },
