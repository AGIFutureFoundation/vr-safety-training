...
export const SIM_CRANE_YARD = {
  ...
  hazards: {
    ...
    "two-block-risk": "You are hoisting past the anti-two-block limit. Running the hook block into the boom tip parts the hoist line or buckles the tip section, and the load falls the moment either one lets go — which is why ANSI/ASME B30.5 and the OSHA rule both put an anti-two-block device on the required list rather than the optional one.",
    "chart-exceeded": "That pick is logged past the chart's rated capacity for this radius and boom angle. A mobile crane's chart already has its safety factor built in, so the plate is the limit and not a target: cranes tip or fail structurally at the edge of the chart, not with a comfortable margin of warning first.",
    "kinked-sling": "That sling has a visible kink in its body. A kink is a stress concentration the sling was never rated for, and ASME B30.9 lists it as a remove-from-service condition precisely because the sling lets go at the kink well under its tag capacity.",
  },
  ...
  steps: [
    {
      ...
      why: "Every control that follows is sized to this specific pick, and OSHA's cranes rule at 29 CFR 1926 subpart CC puts the plan on the crew before the machine is set, not on the office afterwards. A lift plan read once the outriggers are already down is a plan you are checking your own work against.",
    },
    {
      ...
      why: "Rated capacity is published for beams fully extended onto adequate blocking. Retract them part way and the tipping axis moves in toward the boom, so ASME B30.5 treats that as a different chart entirely — one most cabs do not carry. The outriggers are what turn a number on paper into something this ground can hold.",
    },
    {
    ...
    },
    {
      ...
      why: "The chart is read at the actual radius and boom angle for this pick, not the radius from last week's job that felt about the same. Capacity falls away faster than radius grows, so walking the boom out a few feet can cost a fifth of the rating — and under 29 CFR 1926 subpart CC the chart, not the operator's feel for it, is the limit.",
      gauge: {
    ...
    },
    {
      ...
      why: "A taught boundary sitting unsaved is just where the boom happened to point, and it is gone the next time the machine is keyed off. Saving it is what makes the rated capacity indicator actually cut the swing at that line, instead of leaving it to an operator remembering a property line at the end of a twelve-hour shift.",
    },
    {
      ...
      why: "An operational aid is only a control once it has been proved to act, which is why ASME B30.5 has you function-test limiting devices rather than assume them. Verifying a brand-new zone at slow test speed means the first time it stops the boom is a test, not a pick with a load hanging off the hook.",
    ...
    },
    {
      ...
      why: "A sling's tag capacity assumes it is undamaged. ASME B30.9 requires an inspection before every use and lists kinks, broken stitching, heat damage and a missing tag as remove-from-service conditions — the rack always has more than one option, and the rated, sound one is the only correct choice.",
    },
    {
      ...
      why: "A shackle with a worn or bent pin can walk loose under a cyclical load, and ASME B30.26 puts the removal criterion at roughly a tenth of the original diameter worn off the pin or the bow. The stamp tells you what it was rated for; the inspection tells you whether it still is.",
    },
    {
      id: "sling-angle", kind: "gauge", target: "sling-angle-check",
      title: "Measure the sling leg angle",
      cue: "Set the angle finder against a sling leg and commit once the reading is inside the rated range.",
      why: "Leg tension is the share of the load divided by the sine of the angle from horizontal. Two legs at 60 degrees each carry about 58 percent of the load; at 45 they carry 71 percent, and at 30 each leg is carrying the whole load on its own. ASME B30.9 is why the angle gets measured, not eyeballed.",
      gauge: {
        label: "SLING LEG ANGLE FROM HORIZONTAL", speed: 0.6, green: [0.4, 0.74],
        readout: (t) => `${Math.round(20 + t * 70)}° · ×${(0.5 / Math.sin((20 + t * 70) * Math.PI / 180)).toFixed(2)} per leg`,
        missNote: "Outside the rated range. Below 45 degrees each leg is carrying more than seven tenths of the whole load — shorten the spread or take longer legs before this comes off the ground.",
      },
    },
    {
      ...
      why: "One qualified signal person directs the lift and the operator moves on that person's signal alone — with one exception written into 29 CFR 1926 subpart CC: a stop signal from anybody on the site has to be obeyed. Everyone on the pick needs to know who is calling it before the load leaves the ground.",
    },
    {
      ...
      why: "The hook has to come over the load's centre of gravity before anything is taken up. A load hooked off centre swings the instant it clears the ground and puts a side pull into a boom rated only for a vertical one — and that is the moment you have least control over either.",
    },
    {
      id: "tag-line-lift", kind: "track", target: "tag-line-crane", seconds: 6,
      title: "Guide the lift with the tag line",
      cue: "Keep a steady tension on the tag line as the load comes up and swings toward the landing zone.",
      why: "A tag line steers a suspended load without anybody putting a hand on it, and it only works at a steady light tension. Haul on it and you drag the load off plumb into a side load the boom is not rated for; let it go slack and the load keeps whatever swing it already had until something stops it.",
      track: {
        start: 0.12, green: [0.4, 0.62], rise: 0.6, fall: 0.5, drift: 0.11,
        label: "TAG LINE TENSION",
        readout: (v) => (v < 0.4 ? "slack — load swinging" : v > 0.62 ? "hauling load off plumb" : "steady"),
      },
    ...
    },
    {
      ...
      why: "A controlled set-down means the load is sitting flat and taking its own weight before any rigging comes off. Lower it fast and the slings go slack under something that is still rocking, and the rigger who steps in to unhook is doing it under a load that can still shift onto them.",
