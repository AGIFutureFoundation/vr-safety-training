/**
 * The shared fleet, equipment and tool kits, held to what they declare.
 *
 *     node tools/check_fleet.mjs            # gate
 *     node tools/check_fleet.mjs --measure  # print measured counts and footprints
 *
 * WebXR/shared/fleet.js, equipment.js and toolkit.js each export a budget
 * table (FLEET_BUDGET, EQUIPMENT_BUDGET, TOOLKIT_BUDGET) naming, per builder,
 * the mesh count it costs after mergeStatic(), its footprint in metres and the
 * parts a station can animate or register. This builds every entry headlessly
 * and fails on any of:
 *   - the builder throws;
 *   - more meshes than declared, or more than the class ceiling the assets
 *     brief sets (tractor-trailer 40, car 18, forklift 24, excavator 30,
 *     hand tool 4);
 ...
 *
 * mergeStatic() is a no-op on a stub three.js, so the post-merge count is
 * computed here the way mergeStatic() would bake it: every mesh under a baked
 * group (userData.fleetBake) that it is allowed to merge collapses to one mesh
 * per material; decals and anything interactive or ownMaterial stay separate.
 * Footprints come from this file's own stub, which — unlike the shared one —
...
// --- bounds: every geometry carries an axis-aligned box in its own space.
...
class Obj3D {
  ...
  // Apply this object's own transform (scale, rotation in Euler order, position) to points.
  _local(pts){
...
const CEILING = {
  tractorTrailer: 40, "semiTractor": 40, "semiTractor:sleeper": 40,
  sedan: 18, pickup: 18,
  forkliftCounterbalance: 24,
  excavator: 30,
};
const TOOL_CEILING = 4;
...
/** The mesh count mergeStatic() leaves, per baked group. */
...
for (const kit of present) {
  ...
  for (const [key, e] of Object.entries(table)) {
    ...
    if (typeof fn !== "function") { fail(`${kit.file} ${key}: no builder named "${e.build}"`); continue; }
    const root = new suite.THREE.Group();
    let g;
    try { g = fn(root, 0, 0, 0, { ...(e.opts ?? {}) }); } catch (err) { fail(`${kit.file} ${key}: build threw — ${err.message}`); continue; }
    if (!g || !g.isMesh && !g.children) { fail(`${kit.file} ${key}: builder returned nothing`); continue; }
    // A second build with a livery and a turn must also succeed (stations pass both).
    try { fn(new suite.THREE.Group(), 1, 0, -2, { ...(e.opts ?? {}), ry: 0.7, livery: { colour: "#3a6ea5", fleetName: "TEST FLEET", unitNumber: "9" } }); }
    catch (err) { fail(`${kit.file} ${key}: build with opts.livery/ry threw — ${err.message}`); }
    ...
    if (MEASURE) { console.log(`${kit.file.padEnd(20)} ${key.padEnd(26)} meshes ${String(merged).padStart(3)} (raw ${raw})  footprint [${size.map(r2).join(", ")}]  min-y ${r2(b.mn[1])}  centre x ${r2(centre[0])} z ${r2(centre[1])}`); }
    if (typeof e.meshes !== "number") fail(`${kit.file} ${key}: no declared mesh count`);
    else if (merged > e.meshes) fail(`${kit.file} ${key}: ${merged} meshes after merge, declared ${e.meshes}`);
    const ceiling = kit.budget === "TOOLKIT_BUDGET" ? TOOL_CEILING : CEILING[key];
    if (ceiling !== undefined && (e.meshes > ceiling || merged > ceiling)) fail(`${kit.file} ${key}: ${Math.max(merged, e.meshes)} meshes, over the ${ceiling} the assets brief allows`);
    if (!Array.isArray(e.footprint) || e.footprint.length !== 3) fail(`${kit.file} ${key}: footprint must be [width, height, length]`);
    else {
      for (let i = 0; i < 3; i++) {
        const tol = Math.max(0.06, e.footprint[i] * 0.03);
        if (Math.abs(size[i] - e.footprint[i]) > tol) fail(`${kit.file} ${key}: ${"WHL"[i]} measures ${r2(size[i])} m, declared ${e.footprint[i]} m`);
      }
      if (Math.abs(centre[0]) > 0.1 + size[0] * 0.03 || Math.abs(centre[1]) > 0.1 + size[2] * 0.03) fail(`${kit.file} ${key}: footprint not centred (x ${r2(centre[0])}, z ${r2(centre[1])})`);
      if (b.mn[1] < -0.02 && !e.belowGround) fail(`${kit.file} ${key}: geometry below the ground (y ${r2(b.mn[1])})`);
    }
    ...
    for (const name of e.parts ?? []) {
      ...
      if (!list.length) fail(`${kit.file} ${key}: part "${name}" missing from userData.parts`);
      else if (list.some((o) => !o || typeof o.traverse !== "function" || !inTree(o))) fail(`${kit.file} ${key}: part "${name}" is not an object in the builder's tree`);
    }
  }
